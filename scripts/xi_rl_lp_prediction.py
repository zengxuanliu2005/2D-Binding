...
def run_production(n_boot: int, n_jobs: int, out_dir: Path) -> int:
    ...
    # Markdown summary (Tier 2 frontmatter)
    md_path = out_dir / "xi_rl_lp_prediction.md"
    with md_path.open("w") as fp:
        fp.write("---\n")
        fp.write("purpose: B2.4 ξ_RL prediction + Xu 2015 ratio test (auto-generated)\n")
        fp.write("audience: Claude (next session) + essay v2 §5.3 author\n")
        fp.write("status: current\n")
        fp.write(f"generated_by: scripts/xi_rl_lp_prediction.py\n")
        fp.write("derivation: derivation/04_xi_rl_from_lp/\n")
        fp.write("---\n\n")
