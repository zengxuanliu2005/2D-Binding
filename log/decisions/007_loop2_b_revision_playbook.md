...
adr_id: 007
date: 2026-06-02
summary: "Loop 2 protocol: for each senior-data observation, the playbook specifies which derivation step / open_question / calibration row to update — explicit decision tree, no ambiguity"
agent_read_when:
  - cluster/outputs/<date>_round<N>/ just received senior's distilled tarball
...
Loop 2 of the cluster double-loop (ADR 005) goes: senior runs the bundle on
...
ADR 007 consolidates the mapping. For each senior-data observation, ONE
...
| Conflict-Map scenario | Trigger field (senior data) | Primary derivation update | Calibration row to flip | Open-question to close | Re-run? |
|---|---|---|---|---|---|
| **S1** Senior data ≈ PPT s25 (3.64/2.47/-1.18) within bootstrap σ | ΔΔF flex-rigid ≈ 3.64 kBT | none (B2 unaffected) | `closure_methods_consensus`: mark s25 mystery resolved | `derivation/05/05_open_questions.md` Q3 (closed: full data = PPT) | rerun B1 with merged data → expect 3.64 |
| **S2** Senior data ≠ PPT s25 AND ≠ s001 | ΔΔF significantly different from both 3.64 and 5.23 | depends on Lc question | `b2_vs_pptx_re`: append row with new numbers | none yet | rerun B1/B2 with senior's reported Lc if available |
| **S3** Senior σ(R_z) >> 0.35 nm | σ(R_z) rigid full data ≥ 0.6 nm | `derivation/02_z_marginal/05_open_questions.md` Q2 → **resolved** | `b2_vs_measured_xi_rl` factor (a) 1.6× shrinks; `b2_chain_completeness` B2.2 criterion 3 flips ✗→✓ → 19/20 | derivation/02 Q2 | rerun `xi_rl_lp_prediction.py` with senior σ_z (ratio test should stay) |
...
| **S8** Senior K2D(ξ⊥) data is fixed-h slab not fluctuating | confirm via metadata | `derivation/04_xi_rl_from_lp/01_setup.md` (drop σ_p convolution caveat from Q3.c) | `b2_vs_measured_xi_rl`: simplify residual factor (c) | derivation/04 Q3 (c) | no rerun; just simplify essay narrative |
...
within bootstrap σ of senior's own number, S1 fires.

**Actions**:
1. Update `log/calibration/closure_methods_consensus.md`: append row "senior
...
2. Edit `derivation/05_fconf_selfconsistency/05_open_questions.md` Q3: mark
   ...
   confirms PPT s25 = full-data B1, see senior data of <date>".
3. Update `cluster/results/<date>_round<N>_analysis.md` decision section:
   "§0 hypothesis CONFIRMED — data volume was the gap".
4. Essay v2 §4.6 rewrite plan: replace "1.5-2 kBT senior-vs-us mystery" with
...
### S2 — Senior data ≠ PPT AND ≠ s001

**Trigger**: Senior data is inconsistent with both 3.64 (PPT) and 5.23 (our
B1 s25 reimpl).

**Actions**:
1. First check: did senior use a different Lc convention? Add this question
   to `cluster/results/<date>_round<N>_next_actions.md` for user to ask senior.
...
### S3 — Senior σ(R_z) ≫ 0.35 nm

**Trigger**: Senior's chain_coords extracts give σ(R_z) for rigid system that
...
1. Update `log/calibration/b2_vs_measured_xi_rl.md`: footnote on factor (a)
   ...
   true value is ~σ_senior; B2.2 σ_z = 0.625 within X% of measured".
...
5. Optionally rerun `scripts/xi_rl_lp_prediction.py` with K2D computed using
   senior's σ_z (not WLC σ_z) → check ratio test still passes (it should,
   ratio is scale-invariant). New session log.

### S4 — k_a varies > 10% across systems

**Trigger**: Senior data includes per-system k_a_eff from chain_coords; rigid
...
**Trigger**: Slab measurement of K2D,max gives ratios different from senior's
...
### S8 — Senior K2D(ξ⊥) was fixed-h slab

**Trigger**: Confirm via metadata in senior bundle's run_log or by asking.

**Actions**:
1. Update `derivation/04_xi_rl_from_lp/05_open_questions.md` Q3: mark
   factor (c) as "not applicable — senior's ξ_RL fit was on slab data, so
...
If senior data doesn't match any S1-S8 cleanly OR matches multiple ambiguously:
...
3. Update `cluster/results/<date>_round<N>_next_actions.md` with the specific
   question for senior or for user discussion.
